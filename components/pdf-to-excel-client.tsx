...
import { Upload, FileText, Download, AlertCircle, RefreshCcw, Settings, FileArchive, CheckCircle2, Table as TableIcon } from 'lucide-react';
...
type ExtractionMode = 'smart' | 'table' | 'financial';
...
export default function PdfToExcelClient() {
  ...
  const [outputFilename, setOutputFilename] = useState<string>('spreadsheet.xlsx');
  ...
  const handleConvert = async () => {
    ...
    try {
      // Basic extraction for client-side demo
      // In production, this would go to a backend API capable of proper table structure recognition
      ...
      for (let i = 1; i <= pdf.numPages; i++) {
        ...
        // Group items by Y coordinate to form rows (naive approach)
        const rowsMap = new Map<number, any[]>();
        
        for (const item of textContent.items) {
          if ('str' in item && item.str.trim() !== '') {
             // round Y to group nearby items into same row
        ...
        // Sort Y coordinates descending (top to bottom on page)
        ...
        for (const y of sortedY) {
           ...
           // Simple array of string values for the row
           const rowValues = rowItems.map(item => item.str);
        ...
        setConversionProgress(10 + Math.floor((i / pdf.numPages) * 70));
      }
      
      setConversionProgress(90);
      
      if (wb.SheetNames.length === 0) {
        // Create an empty sheet if no text found
        const ws = XLSX.utils.aoa_to_sheet([["No text/tables found in PDF"]]);
      ...
      setOutputFilename(file.name.replace('.pdf', '.xlsx'));
      ...
      <div className="bg-background border-b pt-12 pb-8">
        <div className="container mx-auto px-4">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h1 className="text-3xl font-bold tracking-tight">PDF to Excel Converter</h1>
              <p className="text-muted-foreground mt-1">Convert PDF tables into editable Excel spreadsheets.</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="max-w-4xl mx-auto">
            <div className="bg-card border rounded-2xl shadow-sm overflow-hidden text-center p-8 md:p-12">
              ...
              <p className="text-muted-foreground mb-8 text-lg">
                Your PDF tables have been converted to an editable Excel spreadsheet.
              </p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
            ...
            <div className="lg:col-span-2 bg-card border rounded-2xl shadow-sm flex flex-col">
              ...
              <div className="p-6 flex-grow space-y-4">
                ...
                <label className={`flex items-start gap-4 p-5 rounded-xl border-2 cursor-pointer transition-all ${extractionMode === 'smart' ? 'bg-primary/5 border-primary' : 'hover:bg-muted/50 border-transparent'}`}>
                  <input 
                    type="radio" 
                    name="extractionMode" 
                    checked={extractionMode === 'smart'} 
                    onChange={() => setExtractionMode('smart')}
                    className="mt-1"
                  />
                  <div>
                    ...
                    <div className="text-sm text-muted-foreground mt-1">Automatically extracts text rows and columns (client-side demo).</div>
                  </div>
                </label>
                
                <label className={`flex items-start gap-4 p-5 rounded-xl border-2 cursor-pointer transition-all ${extractionMode === 'table' ? 'bg-primary/5 border-primary' : 'hover:bg-muted/50 border-transparent'}`}>
                  <input 
                    type="radio" 
                    name="extractionMode" 
                    checked={extractionMode === 'table'} 
                    onChange={() => setExtractionMode('table')}
                    className="mt-1"
                    disabled
                  />
                  <div className="opacity-70">
                    ...
                    <div className="text-sm text-muted-foreground mt-1">Extracts only bordered tables. (Requires Server AI).</div>
                  </div>
                </label>
                
                <label className={`flex items-start gap-4 p-5 rounded-xl border-2 cursor-pointer transition-all ${extractionMode === 'financial' ? 'bg-primary/5 border-primary' : 'hover:bg-muted/50 border-transparent'}`}>
                  <input 
                    type="radio" 
                    name="extractionMode" 
                    checked={extractionMode === 'financial'} 
                    onChange={() => setExtractionMode('financial')}
                    className="mt-1"
                    disabled
                  />
                  <div className="opacity-70">
                    ...
                    <div className="text-sm text-muted-foreground mt-1">Optimized for invoices and bank statements. Detects complex layouts.</div>
                  </div>
                </label>
              ...
              <div className="px-6 pb-2 text-xs text-muted-foreground bg-card">
                ...
                Note: Client-side demo uses basic horizontal grouping. Complex tables and merged cells may not be perfectly aligned without a backend OCR/AI layer.
              </div>
              ...
              <div className="p-6 border-t bg-muted/10">
                {isProcessing && (
                  <div className="mb-4">
                    <div className="flex justify-between text-sm mb-1">
                      <span className="text-muted-foreground">Extracting tables...</span>
                ...
                <div className="flex justify-end">
                  ...
                  >
                    {isProcessing ? <RefreshCcw className="w-5 h-5 animate-spin" /> : <TableIcon className="w-5 h-5" />}
